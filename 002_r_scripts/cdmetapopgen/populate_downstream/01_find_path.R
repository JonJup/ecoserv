...
tb_edge = tibble(EdgeID = igraph_nodes[[3]][, 1],
                 Node1  = igraph_nodes[[3]][, 2],
                 Node2  = igraph_nodes[[3]][, 3])
...
int_start = st_nearest_feature(x = st_sites[int_start_site,],
                               y = st_rivers)
# int_end   = st_nearest_feature(x = goal_point,
#                                y = st_rivers)

int_start = st_rivers[int_start, ]$ecoserv_id
int_end   = ch_goal_segment_id
from      = dt_edge[ecoserv_id ==  int_start, Node2]
to        = dt_edge[ecoserv_id ==  int_end,   Node1]

li_sp = shortest_paths(graph = igraph_rivers,
                       from  = from,
                       to    = to)
...
# # in_d1 = st_distance(x = st_sites[int_start_site, ],
# #                     y = filter(st_rivers, ecoserv_id == tb_edge_id_sub[segment_number == 2, ecoserv_id])) %>%
# #         as.numeric()
# # in_d2 = st_distance(x = filter(st_rivers, ecoserv_id == ch_goal_segment_id),
# #                     y = filter(st_rivers, ecoserv_id == tb_edge_id_sub[segment_number == max(segment_number)-1, ecoserv_id])) %>% 
# #         as.numeric()
# # # FILL OUTPUT MATRICES
# #  p_total
# # int_distance =  in_most_segments + in_d2 + in_d1
# # int_distance_p = dnorm(x =int_distance, mean = 51, sd = 9.2) # data: Knaepkens et al (2005) Table 1 
# # ## PRINT LOOP END 
